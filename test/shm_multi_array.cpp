...
mpic++ -O3 -std=c++17 -Wall -Wextra -Wpedantic -Wfatal-errors $0 -o $0x.x -lboost_serialization && time mpirun -np 4 $0x.x $@ && rm -f $0x.x; exit
#endif
...
auto copy_n(
	It1 first, Size n,
	array_iterator<T, 1, boost::mpi3::shm::pointer<T>> d_first
){
...

{
	...
	auto a = A.get_allocator();
	auto b = B.get_allocator();
	swap(a, b);
