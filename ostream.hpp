...
(echo "#include<"$0">" > $0x.cpp) && mpicxx -O3 -std=c++17 -Wfatal-errors -D_TEST_BOOST_MPI3_ALLOCATOR -lboost_mpi -lboost_serialization -lboost_container $0x.cpp -o $0x.x && time mpirun -np 4 $0x.x $@ && rm -f $0x.x $0x.cpp; exit
#endif
...
class ostream : public std::ostream{ // http://stackoverflow.com/a/2212940/225186
	class streambuf : public std::stringbuf{
		...
		public:
		streambuf(std::ostream& strm, communicator const& comm) : output(strm), comm_(comm){}
		virtual int sync(){
			if(comm_.rank()==0){
...
//	communicator const& comm_;
	public:
	ostream(std::ostream& os, communicator const& comm)
	: std::ostream(&buffer), buffer(os, comm){}//, comm_(comm){}
	~ostream(){flush();}
};
...
int boost::mpi3::main(int argc, char* argv[], boost::mpi3::communicator const& world){
	boost::mpi3::ostream cout(std::cout, world);
