...
//	}
	...
	template<
		...
		typename Size, 
		class datatype = detail::datatype<typename std::iterator_traits<ContiguousIterator>::value_type>
	...
	send_request send_init_n(ContIt first, Size count, int dest, int tag = 0);
	template<class ContIt>
	send_request send_init(ContIt first, ContIt last, int dest, int tag = 0);
	...
	template<class InputIt, class Size, 
		class value_type = typename std::iterator_traits<InputIt>::value_type, class datatype = detail::datatype<value_type>, 
		typename = std::enable_if_t<std::is_pod<typename std::iterator_traits<InputIt>::value_type>::value>
	>
	auto pack_n_aux(std::random_access_iterator_tag, 
		InputIt I, Size n, char* out, int max_size = std::numeric_limits<int>::max()
	){
	//	int position = buffer.size(); 
	//	int size = pack_size(it->second, n);
	//	buffer.resize(position + size);
		int position = 0;
		MPI_Pack(std::addressof(*I), n, datatype{}, out, max_size, &position, impl_);
		return out + position;
	//	buffer.resize(position);
	}

	template<class InputIt, class Char, class value_type = typename std::iterator_traits<InputIt>::value_type, class datatype = detail::datatype<value_type>>
	auto unpack_n(InputIt it, int n, Char const* buffer) const{
		int position = 0;
		MPI_Unpack(buffer, n, &position, std::addressof(*it), n, datatype{}, impl_);
		return (Char const*)((char const*)buffer + position);
	}

	template<class T, class datatype = detail::datatype<T>> 
	...
	package make_package(int n = 1024);
...
public:
	...
	template<
		...
		class datatype = detail::datatype<typename std::iterator_traits<IOIterator>::value_type>
	>
	auto send_receive_n_aux(
	...
	template<
		class ContiguousIterator, class Size, 
		typename value_type = typename std::iterator_traits<ContiguousIterator>::value_type,
		class datatype = detail::datatype<value_type>
	> 
	request ireceive_n(ContiguousIterator I, Size count, int source = MPI_ANY_SOURCE, int tag = MPI_ANY_TAG);

	void send_packed_n(void const* begin, int n, int dest, int tag = 0) const{
...
private:
	...
	auto send_n_randomaccess_contiguity(CommunicationMode cm, BlockingMode bm, RandomAccessIterator first, Size n, int dest, int tag){
		return send_n_randomaccess_contiguous_builtin(cm, bm, detail::is_builtin_type<typename std::iterator_traits<RandomAccessIterator>::value_type>{}, first, n, dest, tag);
	}
	template<class CommunicationMode, class BlockingMode, class RandomAccessIterator, class Size, class Ptr = decltype( detail::data(std::declval<RandomAccessIterator>()) )>
	auto receive_n_randomaccess_contiguity(CommunicationMode cm, BlockingMode bm, RandomAccessIterator first, Size n, int dest, int tag){
		return receive_n_randomaccess_contiguous_builtin(cm, bm, detail::is_builtin_type<typename std::iterator_traits<RandomAccessIterator>::value_type>{}, first, n, dest, tag);
	}

	template<class CommunicationMode, class ContiguousIterator, class Size, class ValueType = typename std::iterator_traits<ContiguousIterator>::value_type, class datatype = typename detail::datatype<ValueType> >
...
private:
	...
	template<class It1, class It2, 
		class sendtype = detail::datatype<typename std::iterator_traits<It1>::value_type>,
		class recvtype = detail::datatype<typename std::iterator_traits<It2>::value_type>
	>
...
private:
	...
	auto broadcast_n_category(detail::contiguous_iterator_tag, ContiguousIt first, Size count, int root){
		return broadcast_n_contiguous_builtinQ(detail::is_builtin_type<typename std::iterator_traits<ContiguousIt>::value_type>{}, first, count, root);
	}
	template<class ContiguousIt, typename Size, class sendtype = detail::datatype<typename std::iterator_traits<ContiguousIt>::value_type>>
...
public:
	template<class T>
	void reduce_value(T const& t, T& ret, operation const& op, int root = 0){
	...
	template<class T>
	auto reduce_value(T const& t, operation const& op, int root = 0){
		T ret = T(0);
		reduce_value(t, ret, op, root);
	//	if(rank() == root) return optional<T>(ret);
		return ret;
	}
//	template<class T>
//	T& reduce_value(T& t, operation const& op, int root = 0){
//		T newval; 
//		reduce(std::addressof(t), std::addressof(t)+1, std::addressof(newval), op, root);
//		t = newval;
//		return t;
//	}
	template<class T>
	T all_reduce_value(T const& t, operation const& op){
		T ret = T(0);
		all_reduce_value(t, ret, op);
		return ret;
	}
	template<class It1, class It2>
	auto reduce(It1 first, It1 last, It2 d_first, operation const& op, int root = 0){
...
private:
	template<class ReducePolicy, class It1, class It2>
	auto reduce(ReducePolicy rp, It1 first, It1 last, It2 d_first, operation const& op, int root = 0){
		return reduce_category(rp, typename std::iterator_traits<It1>::iterator_category{}, first, last, d_first, op, root);
	}
	template<class ReducePolicy, class It1, class Size, class It2,
		class V1 = typename std::iterator_traits<It1>::value_type, 
		class V2 = typename std::iterator_traits<It2>::value_type 
	>
	auto reduce_n(ReducePolicy rp, It1 first, Size count, It2 d_first, operation const& op, int root = 0){
		return reduce_n_dispatch(rp, 
			std::integral_constant<bool, 
				detail::is_builtin_type<V1>::value and detail::is_builtin_type<V2>::value
				and detail::is_contiguous<It1>::value and detail::is_contiguous<It2>::value
			>{}, first, count, d_first, op, root
		);
	}
	template<class ReducePolicy, class RandomAccessIt1, class It2>
	auto reduce_category(ReducePolicy rp, std::random_access_iterator_tag, RandomAccessIt1 first, RandomAccessIt1 last, It2 d_first, operation const& op, int root = 0){
	...
	template<class It1, class Size, class It2,
		...
		class V2 = typename std::iterator_traits<It2>::value_type 
	>
	auto reduce_n_dispatch(all_reduce_mode rp, std::true_type, It1 first, Size count, It2 d_first, operation const& op, int root = 0){
		int s = rp(
			detail::data(first)  , 
			detail::data(d_first), count, detail::datatype<V1>::value,
			op.impl_, impl_
		);
		if(s != MPI_SUCCESS) throw std::runtime_error("cannot scatter");
	}
	template<class It1, class Size, class It2,
		...
		class V2 = typename std::iterator_traits<It2>::value_type 
	>
	auto reduce_n_dispatch(reduce_mode rp, std::true_type, It1 first, Size count, It2 d_first, operation const& op, int root = 0){
		int s = rp(
			detail::data(first)  , 
			detail::data(d_first), count, detail::datatype<V1>::value,
			op.impl_,
	...
	template<class It1, class Size, class It2,
		...
		class V2 = typename std::iterator_traits<It2>::value_type 
	>
	auto reduce_n_dispatch(ireduce_mode rp, std::true_type, It1 first, Size count, It2 d_first, operation const& op, int root = 0){
		request ret;
		int s = rp(
			detail::data(first)  , 
			detail::data(d_first), count, detail::datatype<V1>::value,
...
private:
	...
	void scatter_n_dispatch(std::true_type, ContIt1 first, Size count, ContIt2 d_first, int root){
		...
		int s = MPI_Scatter(
			detail::data(first), count/size(), detail::datatype<V1>::value,
			detail::data(d_first), count/size(), detail::datatype<V2>::value,
			root, impl_
		);
		if(s != MPI_SUCCESS) throw std::runtime_error("cannot scatter");
	}
	...
	void scatter_from_n_dispatch(std::true_type, ContIt2 d_first, Size count, ContIt1 first,  int root){
		if(count % size() != 0) throw std::runtime_error("not matching size for scatter");
		int s = MPI_Scatter(
			detail::data(first), count, detail::datatype<V1>::value,
			detail::data(d_first), count, detail::datatype<V2>::value,
			root, impl_
		);
		if(s != MPI_SUCCESS) throw std::runtime_error("cannot scatter");
	}
	...
	void scatter_n(
		...
		MPI_Scatter(
			(void*)(&*first) , send_count/size()   , detail::datatype<IT>::value, 
			(void*)(&*result), receive_count/size(), detail::datatype<OT>::value, 
			root, impl_
		);
	}
#endif
...
public:
	...
	template<class T, class It> 
	void gather_value(T const& t, It first, int root = 0){gather_n(std::addressof(t), 1, first);}

	template<class It1, class Size, class It2>
	auto gather_n(It1 first, Size count, It2 d_first, int root = 0){return gather_n(gather_mode{}, first, count, d_first, root);}
	template<class It1, class It2>
	auto gather(It1 first, It1 last, It2 d_first, int root = 0){return gather(gather_mode{}, first, last, d_first, root);}
	template<class It1, class Size, class It2>
	...
	template<class It1, class It2>
	auto all_gather(It1 first, It1 last, It2 d_first){return gather(all_gather_mode{}, first, last, d_first);}
private:
	...
	auto gather(GatherPolicy gp, It1 first, It1 last, It2 d_first, int root = 0){
...
/*	friend group _union(group const& g1, group const& g2){
	...
	int is_null() const{return MPI_COMM_NULL == impl_;}
//	operator bool() const{return not is_null();}
	int size() const{
		if(is_null()) return 0;
	...
	boost::mpi3::equality compare(group const& other) const{
		int result;
		MPI_Group_compare(impl_, other.impl_, &result);
		return static_cast<boost::mpi3::equality>(result);
	}
	friend boost::mpi3::equality compare(group const& self, group const& other){return self.compare(other);}
...
#if 0
template<class Iterator>
communicator communicator::include(Iterator it1, Iterator it2) const{
	group original(*this);
	group _new = original.include(it1, it2);
	return communicator(*this, _new);
}
#endif
