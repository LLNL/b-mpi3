...
auto inline string(enum error err) {
	...
	int const s = MPI_Error_string(static_cast<int>(err), estring.data(), &len);  // do not use the MPI_() macro or mpi3::call here because they need this in the first place to capture the error message
